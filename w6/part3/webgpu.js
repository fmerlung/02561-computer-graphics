...
async function main() {
    ...
    const generateMipmap = (() => {
        ...
        return function (device, texture) {
            if (!module) {
                module = device.createShaderModule({
                    ...
                    @vertex fn main_vs(@builtin(vertex_index) vertexIndex : u32) -> VSOutput {
                        var pos = array<vec2f, 4>(vec2f(-1.0, 1.0), vec2f(-1.0, -1.0), vec2f(1.0, 1.0), vec2f(1.0, -1.0));
                        var uv = array<vec2f, 4>(vec2f(0.0, 0.0), vec2f(0.0, 1.0), vec2f(1.0, 0.0), vec2f(1.0, 1.0));
                        var vsOutput: VSOutput;
                        vsOutput.position = vec4f(pos[vertexIndex], 0.0, 1.0);
                        vsOutput.texcoord = uv[vertexIndex];
                        return vsOutput;
                    }
                    @fragment fn main_fs(@location(0) texcoord: vec2f) -> @location(0) vec4f {
    ...
    function frame() {
        ...
        var eye = vec3(
            radius * Math.sin(time * 0.5),
            1.0,
            radius * Math.cos(time * 0.5)
        );
