...
async function main() {
    ...
    const lightPos = vec4(10.0, 10.0, 10.0, 1.0); 
    const Le = 1.0;     
    const La = 0.2;     
    const ks = 0.5;     
    const shininess = 100.0;
    
    const params = new Float32Array([Le, La, ks, shininess]);
    ...
    function frame() {
        ...
        device.queue.writeBuffer(uniformBuffer, 192, new Float32Array(lightPos));
        device.queue.writeBuffer(uniformBuffer, 208, new Float32Array([...eye, 1.0])); // Camera Pos
        device.queue.writeBuffer(uniformBuffer, 224, params);
